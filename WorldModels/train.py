...
def worker(weights, seed, train_mode_int=1, max_len=-1):
    ...
    if train_mode_int == 1:  # train with DREAM env
        ...
        reward_list, t_list = simulate(controller, test_env,
                                       train_mode=train_mode, render_mode=False, num_episode=num_test_episode,
...
def evaluate_batch(model_params, train_mode, max_len=-1):
    ...
    if args.env_name == 'DoomTakeCover-v0' and train_mode == -1:  # can't run the real environment in parallel for doom
        ...
        rewards_list, t_list = simulate(controller, test_env,
                                        train_mode=train_mode, render_mode=False, num_episode=100, seed=0,
                                        max_len=max_len)  # run exactly 100 episodes b/c we cant run in parallel
    else:
        ...
        if (t % eval_steps == 0):  # evaluate on the dream environment with the best agent in the population
            ...
            if config_args.dream_env == True:
                reward_eval_list = evaluate_batch(model_params_quantized, max_len=-1,
                                                  train_mode=0)  # evaluate in dream environment
            else:
                reward_eval_list = evaluate_batch(model_params_quantized, max_len=-1,
                                                  train_mode=-1)  # evaluate in real environment
            ...
            if (len(eval_log) == 1 or reward_eval > best_reward_eval):
                ...
                if config_args.dream_env == True:
                    reward_real_eval_list = evaluate_batch(best_model_params_eval, max_len=-1,
                                                           train_mode=-1)  # evaluate in REAL environment
...
def main(args):
    global optimizer, num_episode, num_test_episode, eval_steps, num_worker, num_worker_trial, antithetic, seed_start, \
           retrain_mode, cap_time_mode, env_name, exp_name, batch_mode, config_args
...
def mpi_fork(n):
  ...
  """
    ...
    if os.getenv("IN_MPI") is None:
        ...
        subprocess.check_call(["mpirun", "--allow-run-as-root", "-np", str(n), sys.executable] + ['-u'] + sys.argv,
                              env=env)
        return "parent"
    else:
